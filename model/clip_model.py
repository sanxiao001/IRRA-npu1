...
def build_CLIP_from_openai_pretrained(name: str, image_size: Union[int, Tuple[int, int]], stride_size: int, jit: bool = False, download_root: str = None):
    ...
    if name in _MODELS:
        ...
        model_path = _download(_MODELS[name], download_root or os.path.expanduser("./model/clip_pretrain"))
    ...
    try:
        # loading JIT archive
        model = torch.jit.load("/home/ma-user/work/IRRA-npu1/model/clip_pretrain/ViT-B-16.pt", map_location=device)
        # model = torch.jit.load("/home/ma-user/work/IRRA-npu1/model/Meta-Transformer_large_patch14_encoder.pth", map_location=device)
        state_dict = None
        ...
        if jit:
            ...
            jit = False
        state_dict = torch.load("vit_load.pt", map_location=device)
